...
# def loss_disp_unsupervised(img_left, img_right, disp=None, valid_mask=None, mask=None, vr=True): #loss_disp_unsupervised(img_left, img_right, disp, valid_mask=None, mask=None)
...
#     return loss
def loss_disp_unsupervised(img_left, image_warped, mask):
    b, _, h, w = img_left.shape

    loss = 1.0 * L1Loss(image_warped[mask], img_left[mask]) + \
           0.01 * ((1 - ssim(img_left, image_warped, window_size=11)) / 2)[mask].mean()
    return loss
